...
(() => {
  const KEYS = {
    ...
    authExpiresAt: "medicareAuthExpiresAtV4"
  };
  ...
  function clearPrivateState() {
    ...
    localStorage.removeItem(KEYS.authExpiresAt);
  }
  ...
  function setAuthExpiry() {
    localStorage.setItem(KEYS.authExpiresAt, String(Date.now() + AUTH_DURATION_MS));
  }

  function authExpired() {
    const expiresAt = Number(localStorage.getItem(KEYS.authExpiresAt) || 0);
  ...
  async function currentUser() {
    try {
      ...
      if (data.logged_in && !localStorage.getItem(KEYS.authExpiresAt)) setAuthExpiry();
      if (!data.logged_in) localStorage.removeItem(KEYS.authExpiresAt);
      return data;
    } catch {
      return { logged_in: false };
    }
  ...
  function ensureAuthModal() {
    ...
    $("#sharedLoginForm").addEventListener("submit", async (event) => {
      ...
      showToast("Đăng nhập thành công. Phiên đăng nhập được lưu trong 30 ngày.", "success");
  ...
  async function bindAccountButton(button) {
    ...
    const refresh = async () => {
      ...
      if (nameNode) {
        nameNode.textContent = data.logged_in
          ? (data.user?.full_name || "Tài khoản")
          : "Đăng nhập / Đăng ký";
        const subtitle = nameNode.parentElement?.querySelector("small");
        if (subtitle) subtitle.textContent = data.logged_in ? "Tài khoản của tôi" : "Bấm để tiếp tục";
        const avatar = button.querySelector(".account-avatar");
        if (avatar) avatar.textContent = data.logged_in ? initials(data.user?.full_name) : "↪";
      } else {
        button.textContent = data.logged_in ? "⇥ Đăng xuất" : "⇥ Đăng nhập / Đăng ký";
      }
      return data;
    };
